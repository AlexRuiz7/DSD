 ...
 */
 ...
 MongoClient.connect("mongodb://localhost:27017/domotica", function(err, db) {
 	if(err)
 		throw err;
  ...
  dbo.createCollection("anti_incendios", function(err, collection){
		if(!err){
			console.log("Colección creada en Mongo: " + collection.collectionName);
		}
	});
  ...
  io.sockets.on('connection',function(socket) {
    ...
    /*** Inserción de colección Persianas ***/
      /** Valores **/
		socket.on('enviar-datos-humo', function (datos) {
			dbo.collection("anti_incendios").insert({valor:datos[0], minimo:datos[1], maximo:datos[2]},
       {safe:true}, function(err, result) {
        if (!err) {
          console.log("Insertado en [anti_incendios]: {valor:"+datos[0]+", minimo:"+datos[1]+", maximo:"+datos[2]+"}");
          io.sockets.emit('log', getTimeStamp() + " - Modificados valores de humo");
        }
				else
					console.log("Error al insertar datos en la colección.");
			});
    ...
    socket.on('slider_3', function (datos) {
			dbo.collection("anti_incendios").insert({estado:datos}, {safe:true}, function(err, result) {
        if (!err) {
          console.log("Insertado en [anti_incendios]: {estado:"+datos+"}");
          io.sockets.emit('log', getTimeStamp() + " - anti_incendios " + datos);
        }
				else
					console.log("Error al insertar datos en la colección.");
			});
    ...
    /*** Examen ***/
    socket.on ('fuego', function (sensor) {
      if (sensor == "humo"){
        var log = "Aviso: se ha detectado un porcentage de humo elevado";
        console.log (log);
        io.sockets.emit ('aviso', log);
      }
      else {
        var log = "Aviso: apagando sistema de aire acondicionado";
        console.log (log);
        io.sockets.emit ('aviso', log);
      }
    });
